...
public:
    ...
    IntParticleEmitter(Drawable* _model, int number, glm::vec3 pos);
    //IntParticleEmitter(Drawable* _model, int number);
	void changeParticleNumber(int new_number);

	void renderParticles(int time = 0);
	virtual void updateParticles(float time, float dt, glm::vec3 camera_pos, glm::vec3 new_emitter_pos) = 0;
