...
IntParticleEmitter::IntParticleEmitter(Drawable* _model, int number, glm::vec3 pos) {
//IntParticleEmitter::IntParticleEmitter(Drawable* _model, int number) {
    model = _model;
    number_of_particles = number;
    //emitter_pos = glm::vec3(0,10,0);
    emitter_pos = pos + glm::vec3(0, 1.0f, 0);
    ...
    lives.resize(number_of_particles, 1.0f);
    distances.resize(number_of_particles, 0.0f);
...
{
    //Billboard looks at (0,0,1) by dafault.
...
void IntParticleEmitter::bindAndUpdateBuffers()
{
    ...
    glBindBuffer(GL_ARRAY_BUFFER, lives_buffer);
    glBufferData(GL_ARRAY_BUFFER, number_of_particles * sizeof(float), NULL, GL_STREAM_DRAW); // Buffer orphaning and reallocating to avoid synchronization, see https://www.khronos.org/opengl/wiki/Buffer_Object_Streaming
    glBufferSubData(GL_ARRAY_BUFFER, 0, number_of_particles * sizeof(float), &lives[0]); //Sending data

    glBindBuffer(GL_ARRAY_BUFFER, distances_buffer);
    glBufferData(GL_ARRAY_BUFFER, number_of_particles * sizeof(float), NULL, GL_STREAM_DRAW); // Buffer orphaning and reallocating to avoid synchronization, see https://www.khronos.org/opengl/wiki/Buffer_Object_Streaming
    glBufferSubData(GL_ARRAY_BUFFER, 0, number_of_particles * sizeof(float), &distances[0]); //Sending data
}
...
void IntParticleEmitter::configureVAO()
{
    ...
    glVertexAttribPointer(13, 1, GL_FLOAT, GL_FALSE, 0, NULL);
